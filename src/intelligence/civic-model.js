...
export function createCivicCity(){
 const root=new T.Group(),assets={},locations={},overlays=new T.Group();root.add(overlays);
 const p={white:material('#eeeade'),stone:material('#d6d3c5'),edge:material('#b8bfae'),glass:material('#427e92',{metalness:.25,roughness:.3}),blue:material('#246c96'),deep:material('#173c50'),teal:material('#178f94'),road:material('#6b7778'),mark:material('#e6e8df'),grass:material('#679452'),leaf:material('#54864c'),leaf2:material('#91ab62'),wood:material('#b3986a'),roof:material('#af7860'),water:material('#36a5b7',{metalness:.2,roughness:.28}),solar:material('#245678',{metalness:.3,roughness:.27}),lime:material('#b4d945'),orchid:material('#9253ad'),warning:material('#edb64c')};
 ...
 function tree(g,x,z,s=1){cylinder(g,.10*s,1.2*s,x,.6*s+.6,z,p.wood,6);sphere(g,.75*s,x,1.7*s+.6,z,((Math.round(x+z)%3)===0?p.leaf2:p.leaf),[1,1.35,1]);}
 function grove(g,x,z,n,axis='x'){for(let i=0;i<n;i++)tree(g,x+(axis==='x'?i*2:0),z+(axis==='z'?i*2:0),.8+(i%3)*.12);}
 function district(id,x,z,w,d,label){const g=new T.Group();g.position.set(x,0,z);g.userData.asset=id;root.add(g);assets[id]=g;locations[id]=[x,6,z];g.name=label;box(g,w,.55,d,0,.05,0,p.white).renderOrder=-20;box(g,w-.5,.12,d-.5,0,.38,0,p.grass).renderOrder=-19;return g;}
 ...
 function block(g,w,d,floors,x=0,z=0,m=p.white){box(g,w,floors*2.2,d,x,.65+floors*1.1,z,m);windows(g,w,d,floors,x,z);box(g,w+.35,.2,d+.35,x,.8+floors*2.2,z,p.stone);}
 ...
 box(root,88,1,66,0,-.7,0,p.white).renderOrder=-40;box(root,87,.16,65,0,-.1,0,p.road).renderOrder=-35;
 ...
 for(const x of [-7.4,7.4])box(civic,.2,.7,8.4,x,5.65,-1,p.white);
 for(const z of [-5,3])box(civic,15,.7,.2,0,5.65,z,p.white);
 ...
 cylinder(tourism,.9,.22,-1,.7,0,p.white);tube(tourism,[[-1,.8,0],[-1,3.6,0],[-1.7,1.1,.3]],.065,p.white);block(tourism,4,3,1,6,-4);roof(tourism,4.4,3.5,6,3.12,-4);
 const industry=district('industry',-2,-23,25,18,'Industry');path(industry,22,15,0,0);block(industry,11,7,2,-4,1,p.stone);box(industry,12,.25,8,-4,5.48,1,p.deep);
 ...
 for(const x of [-7,1,9]){cylinder(energy,.18,9,x,5,-5,p.white,12);sphere(energy,.32,x,9.5,-5,p.white);for(let i=0;i<3;i++){const blade=box(energy,.24,3.5,.10,x,0,-5,p.white);const a=i*Math.PI*2/3;blade.position.y=9.5+Math.cos(a)*1.6;blade.position.x=x+Math.sin(a)*1.6;blade.rotation.z=-a;}}
 ...
 for(const x of [-6,-3,0]){box(hospital,1.3,.6,1.7,x,7.75,-2,p.stone);cylinder(hospital,.35,.08,x,8.1,-2,p.deep,12);}
 for(const x of [-8.3,2.3])box(hospital,.15,.55,7,x,7.85,-1,p.white);
 for(const z of [-4.4,2.4])box(hospital,10.6,.55,.15,-3,7.85,z,p.white);
 locations.hospital=[29,9,1];
 const school=district('school',29,24,24,17,'Education');block(school,10,4,2,-4,-4,p.white);roof(school,11,5,-4,5.32,-4);block(school,5,7,2,7,-1,p.white);roof(school,6,8,7,5.32,-1);
 ...
 box(water,22,.22,14,0,.62,0,p.water).renderOrder=-18;
 for(const x of [-9,-4.5,0,4.5,9]){box(water,.9,3,2.5,x,2.1,1,p.stone);box(water,1.8,.32,3.5,x,3.68,1,p.white);}
 box(water,22,.4,2.1,0,3.1,1,p.stone);for(let x=-8;x<=8;x+=4){box(water,3.3,1.3,.2,x,1.5,1,p.deep);box(water,3,.1,5,x,.94,5,p.white).material=material('#c0e4e7',{transparent:true,opacity:.7});}
 ...
 const housing=district('housing',-30,24,22,17,'Neighbourhood');for(const [x,z] of [[-5,-4],[4,-4],[-5,4],[4,4]]){block(housing,5,4,1,x,z,p.white);roof(housing,5.6,4.6,x,3.12,z);tree(housing,x+3,z,.8);}
 ...
 const dataRoutes=new T.Group();root.add(dataRoutes);for(const [x,z] of [[-30,1],[-2,1],[29,1],[-2,-23]])tube(dataRoutes,[[-53,1.5,1],[-46,1.5,1],[-43,1.5,z],[x,1.5,z]],.08,p.teal);
 // Road vehicles and roadside sensors use fixed positions, not scroll animation.
 for(let i=0;i<10;i++){const x=-38+i*8;box(root,1.5,.55,.8,x,.55,13.5,i%3?p.white:p.blue);box(root,.8,.3,.72,x,.95,13.5,p.glass);}
 const sensorLayer=new T.Group();root.add(sensorLayer);for(const [id,g] of Object.entries(assets)){if(id==='command')continue;const [x,,z]=locations[id];cylinder(sensorLayer,.10,3.5,x+7,2.2,z+4,p.deep,8);cylinder(sensorLayer,.34,.3,x+7,4,z+4,p.lime,12);}
 const risk=new T.Mesh(new T.PlaneGeometry(23,16),new T.MeshBasicMaterial({color:'#8267c4',transparent:true,opacity:.24,side:T.DoubleSide,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2}));risk.rotation.x=-Math.PI/2;risk.position.set(-2,4.45,22);overlays.add(risk);
 const warning=new T.Mesh(new T.OctahedronGeometry(.8),p.warning);warning.position.set(-2,6.5,24);overlays.add(warning);
 function setStage(id){dataRoutes.visible=!['overview','fragmented'].includes(id);sensorLayer.visible=['iot','flood','priorities','ai','roadmap'].includes(id);risk.visible=['flood','ai'].includes(id);warning.visible=risk.visible;risk.position.y=id==='ai'?6:4.45;}
 setStage('overview');root.updateMatrixWorld(true);return {root,assets,locations,setStage};
}
