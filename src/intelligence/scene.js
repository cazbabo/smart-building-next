...
export async function mountScene(host,{select,manual,ready}){
 const scene=new T.Scene();const city=createCity();scene.add(city.root);
 ...
 catch{webgl=false;renderer=new SVGRenderer();renderer.setQuality('low');sun.intensity=1.6;scene.children.find(o=>o.isHemisphereLight).intensity=1.8;}
 ...
 const targets={all:[0,5,0],hub:[0,5,3],office:[-16,12,-18],water:[21,4,3],traffic:[5,4,-3],park:[0,3,21],residence:[-29,7,8]};
 ...
 function update(s,reduced=false){state=s;city.update(s);const c=chapters[s.index];const f=s.focus;const nextSpan=s.index===5?82:c.span;const before=s.index===5?(s.useIndex?useCases[s.useIndex-1].focus:chapters[4].focus):s.index===6?useCases[4].focus:chapters[Math.max(0,s.index-1)].focus;const local=s.index===5?s.u:s.local;const t=reduced?1:Math.min(1,local/.2);const eased=t*t*(3-2*t);const prevSpan=s.index===5?(s.useIndex?82:chapters[4].span):s.index===6?82:chapters[Math.max(0,s.index-1)].span;canonical(f,prevSpan+(nextSpan-prevSpan)*eased,before,eased);dirty=true;}
 function render(now){frame=requestAnimationFrame(render);if(document.hidden||!dirty||now-last<(webgl?16:110))return;last=now;const begin=performance.now();renderer.render(scene,camera);dirty=false;if(webgl&&!low&&performance.now()-begin>45){renderer.setPixelRatio(1);renderer.shadowMap.enabled=false;low=true;dirty=true;}}
 canonical('all',112);render(0);ready(webgl?'3D ready':'Lightweight 3D view');
