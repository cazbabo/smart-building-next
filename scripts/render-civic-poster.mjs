...
const scene=new T.Scene(),city=createCivicCity();scene.add(city.root);scene.add(new T.AmbientLight(0xdddddd));const sun=new T.DirectionalLight(0xfff5ff,.8);sun.position.set(-40,85,45);scene.add(sun);
const camera=new T.OrthographicCamera(-72.5,72.5,54.375,-54.375,.1,400);const target=new T.Vector3(-15,4,0);camera.position.copy(target).add(new T.Vector3(102,100,102));camera.lookAt(target);
